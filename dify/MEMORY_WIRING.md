...
目标：将 `workflow/memory-normalizer.js` 的已测试逻辑同步到 Dify 草稿。多病例歧义和 10 轮过期已完成云端回归；权限撤销仍是发布前阻断项。
...
当前验收状态：本地归一化 `9/9`、记忆契约 `15/15`、Dify 核心记忆 `9/9`、病例错误安全 `5/5`；草稿未发布。

多病例歧义使用 `A20260001` 与 `A20260003`；两者都映射到 `USR-DOC-001` 有权访问的 Mock 病例。`A20260002` 保留为无权限边界数据。
